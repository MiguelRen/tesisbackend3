  ...
  try {
    ...
    const sql =
      "INSERT INTO tabquarter \
        ...
        values \
        (?,?,?)";
    const data = [req.body.quarterPeriod, req.body.trimIni, req.body.trimEnd];
    // console.log(data);
...
export const cur = async (req, res) => {
  ...
  try {
    ...
    if (!exist) {
      ...
      const data = [exist.quaQuarterIni, exist.quaQuarterEnd];
    ...
    //   const [result, metadata] = await connection.query(sql, data);
      const [result, metadata] = await pool.query(sql, data);
      // console.log(result);
      connection.release;
...
export const existed = async (req, res) => {
  try {
    const sql =
      "\
        select * from tabquarter \
        where quaquarterstart = (select max (quaquarterstart) from tabquarter) ;";
